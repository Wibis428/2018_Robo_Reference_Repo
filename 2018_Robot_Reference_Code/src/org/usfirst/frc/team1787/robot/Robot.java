...
public class Robot extends TimedRobot {
	...
	/**
	 * Determines which mode the shooter is in. 0 = manual control, 1 = automatic
	 * target tracking & shooting
	 */
	private int shooterControlMode = 0;

	/**
	 * If tuning mode is active, this variable determines what exactly is being
	 * tuned
	 */
	...
	/*
	 * These subsystems are normally controlled collectively through the Shooter
	 * class, but they are included here individually to tune PID loops for each
	...
	public void robotInit() {
		...
		/*
		 * TODO: 4) finish going through all talon config features 5) practice working
		 * with a sensor that's attatched to the talon 6) figure out if it's possible to
		 * use a gyro with a talon / investigate Pidgeon IMU 7) Test out using the
		 * follow feature with talons 8) See what code structure would be like if you
		 * have the talons control pretty much everything 9) actually use
		 * TalonConfigurer everywhere.
		 * 
		 * 9) Review multi-threading for img processing 10) review img processing code
		 * and see if cleanup is necessary
		 * 
		 * 11) Review what's up with the new network tables protocols 12) Finish setting
		 * up the dashboard so that all values show
		 * 
		 * 13) make arcadeDrive consistent with what I learned in robotics + fix
		 * documentation.
		 */
	...
	/*
	 * While the robot is on, it can be in one of the following modes at a time: 1)
	 * teleop (driver control from driver station) 2) autonomous (no driver control,
	 * fully autonomous) 3) test mode (intended to be helpful for testing, but I
	 * find it annoying) 4) disabled (robot does nothing)
	 * 
	 * Each of these modes have an associated init() function and an associated
	 * periodic() function. The init() function is run once upon entering a given
	 * mode, then the periodic() function is looped until a different mode is
	 * entered.
	 * 
	 * For example: Changing the current mode from disabled to teleop will cause
	 * teleopInit() to be run once, and then teleopPeriodic() to be looped until a
	 * different mode is entered.
	 */
	...
	public void teleopPeriodic() {
		...
		// Cameras (note that most img processing code is already called by
		// shooter.fullAutoShooting())
	...
	public void runTuningCode() {
		...
		/*
		 * tuningMode = 0 = turret PID tuning tuningMode = 1 = flywheel PID tuning
		 * tuningMode = 2 = HSV filter tuning tuningMode = 3 = contour filter tuning
		 */
		if (tuningMode == 0) {
			if (!shooter.pidIsEnabled()) {
